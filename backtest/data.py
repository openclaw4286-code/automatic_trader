...
Results are cached as parquet under logs/backtest_cache/.

Why parquet: reading 30 days of 3m bars is ~14k rows × 6 cols, which
loads in <50ms from parquet vs the ~3s a paginated network fetch costs.
That difference matters because the grid sweep replays the same data
100+ times across configurations.
"""
...
def _cache_path(symbol: str, timeframe: str, days: int) -> Path:
    ...
    return CACHE_DIR / f"{safe}__{timeframe}__{days}d.parquet"
